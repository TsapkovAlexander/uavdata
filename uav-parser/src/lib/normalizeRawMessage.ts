...
const shortFormRe =
  /^(DEP|ARR)\s*-\s*([A-Z0-9\-]+)\s*-\s*([A-Z]{4})(\d{4})\s*-\s*([A-Z]{4})(\d{4})(?:\s|$)/i;
...
function joinBrokenLines(s: string): string {
  // склеиваем "\r?\n-" и просто переносы
  return s.replace(/\r?\n-\s*/g, "-").replace(/\r?\n+/g, " ");
}
...
function extractUID(s: string): string | null {
  ...
  if (m3) return `SID${m3[1]}`;

  return null;
}

// Простая детерминированная «заглушка», если ничего не нашли
function fallbackUID(s: string): string {
...
function hasDOF(s: string): boolean {
  return /\bDOF\/\d{6}\b/.test(s);
}

// Добавить RMK/UID если его нет
function ensureUID(s: string): string {
  if (/\bRMK\/UID\b/i.test(s)) return s;
  const uid = extractUID(s) ?? fallbackUID(s);
  return `${s} RMK/UID ${uid}`;
}

export function normalizeRawMessage(raw: string): string {
