...
export default async function flightsRoutes(app: FastifyInstance) {
  ...
  app.get("/flights", async (req, reply) => {
    ...
    if (q.from) query = query.gte("dof", q.from);
    if (q.to)   query = query.lte("dof", q.to);
    if (q.sid)  query = query.eq("sid", q.sid);
    if (q.region_id) {
