...
export function dof6ToISO(dof6?: string | null): string | null {
  if (!dof6) return null;
  ...
  const d = dayjs(iso);
  if (!d.isValid()) return null; // отсекаем 2025-13-40 и т.д.
  return iso;
}
 ...
 */
export function hhmmToTime(hhmm?: string | null): string | null {
  if (!hhmm) return null;
...
export function computeDurationMin(depTime?: string | null, arrTime?: string | null): number | null {
