...
import type { FlightUnified } from "../models/unified.js";
...
export async function upsertFlights(flights: FlightUnified[]) {
  ...
  for (const f of flights) {
    ...
    const { data, error } = await supa.rpc("upsert_flight_norm", { f: payload as any });
    if (error) {
      fail++;
