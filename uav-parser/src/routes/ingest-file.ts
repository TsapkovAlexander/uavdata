...
async function extractTextsFromXlsx(buf: Buffer): Promise<string[]> {
  const { read, utils } = await import("xlsx");
  const wb = read(buf, { type: "buffer" });
  const out: string[] = [];
  for (const sheetName of wb.SheetNames) {
    const ws = wb.Sheets[sheetName];
    const rows: any[][] = utils.sheet_to_json(ws, { header: 1, raw: false });
...
function coordsToStr(obj?: { lat?: number; lon?: number }): string | undefined {
  if (!obj || typeof obj.lat !== "number" || typeof obj.lon !== "number") return undefined;
  // формат «lat,lon» на всякий — если нет исходной строки
  return `${obj.lat},${obj.lon}`;
}

function ensurePoints<T extends Record<string, any>>(f: T): T {
  if (!f.dep_point) {
...
function missingFieldsSummary(f: any) {
...
export default async function ingestFileRoutes(app: FastifyInstance) {
  app.post("/ingest-file", async (req, reply) => {
    ...
    if (!sliced.length) return reply.code(400).send({ error: "no recognizable messages in file" });
    ...
    const previewMerged: any[] = [];
    const previewSane: any[] = [];
    const previewMissing: any[] = [];
    ...
    for (let i = 0; i < adapted.length; i += CHUNK) {
      ...
      if (previewMerged.length < 5) previewMerged.push(...hydrated.slice(0, 5 - previewMerged.length));

      // sanity
      const sane = hydrated.filter((f: any) => {
        const ok = isSane(f);
        if (!ok && previewMissing.length < 12) previewMissing.push(missingFieldsSummary(f));
        return ok;
      });
      flightsSaneTotal += sane.length;

      if (previewSane.length < 5) previewSane.push(...sane.slice(0, 5 - previewSane.length));
