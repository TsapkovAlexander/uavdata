...
export function parseCoord(token) {
    ...
    if (m) {
        ...
        const lat = dmsToDec(+latD, +latM, +latS, latH);
        const lon = dmsToDec(+lonD, +lonM, +lonS, lonH);
    ...
    if (m) {
        ...
        const lat = dmsToDec(+latD, +latM, 0, latH);
        const lon = dmsToDec(+lonD, +lonM, 0, lonH);
        if (isValidLat(lat) && isValidLon(lon))
            return { lat, lon, src };
    }
    // Вариант B: отдельные широта и долгота с N/S и E/W, произвольные пробелы/разделители
    const latToken = findFirst(cleaned, /(\d{1,2}(?:[\s:]*\d{1,2}){0,2}(?:[\s:]*\d{1,2})?)\s*([NS])\b/);
    const lonToken = findFirst(cleaned, /(\d{1,3}(?:[\s:]*\d{1,2}){0,2}(?:[\s:]*\d{1,2})?)\s*([EW])\b/);
    if (latToken && lonToken) {
        const lat = parseOneCoord(latToken.value, latToken.hemi, /*isLat*/ true);
        const lon = parseOneCoord(lonToken.value, lonToken.hemi, /*isLat*/ false);
    ...
    if (latDec && lonDec) {
        const lat = applyHemisphere(parseFloat(latDec[1].replace(",", ".")), latDec[2]);
        const lon = applyHemisphere(parseFloat(lonDec[1].replace(",", ".")), lonDec[2]);
        if (isValidLat(lat) && isValidLon(lon))
            return { lat, lon, src };
    }
    ...
    if (signedPair) {
        ...
        if (isValidLat(a) && isValidLon(b))
            return { lat: a, lon: b, src };
        // И как lon,lat (редко встречается, но перестрахуемся)
        if (isValidLat(b) && isValidLon(a))
            return { lat: b, lon: a, src };
    }
    return null;
}
 ...
 */
function parseOneCoord(value, hemi, isLat) {
    ...
    if (digits.length === 3) {
        ...
        if (isLat ? isValidLat(signed) : isValidLon(signed))
            return signed;
        return null;
    }
    else {
    ...
    if (d < 0 || d > maxD)
        return null;
    if (d === maxD && (m > 0 || s > 0))
        return null; // 90°00'00"N допустимо, 90°10' — нет
