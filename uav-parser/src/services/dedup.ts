...
function firstReg(f: any): string | null {
  const r = Array.isArray(f.reg)
    ? f.reg.find((x: string) => x && x !== "ZZZZZ")
    : null;
  return r || null;
}
...
export function mergeFlights(a: any, b: any) {
  // Берём x если он есть, иначе y
  const pick = (x: any, y: any) => (x != null ? x : y);

  // Мерж массивов с нормализацией и uniq
  const uniqArray = (arr: any[] = []) =>
    Array.from(new Set(arr.map((x) => (typeof x === "string" ? x.trim() : x)))).filter(Boolean);

  // Для оператора мержим name и телефоны, если оба есть
  const mergeOperator = (op1: any, op2: any) => {
    if (!op1 && !op2) return undefined;
    if (!op1) return op2;
    if (!op2) return op1;
  ...
  const pickCoord = (c1: any, c2: any) => {
    if (!c1) return c2;
    if (!c2) return c1;
    if (c1.src && c2.src) {
      if (c1.src.length > c2.src.length) return c1;
      if (c2.src.length > c1.src.length) return c2;
    }
    return c1 || c2;
  };
