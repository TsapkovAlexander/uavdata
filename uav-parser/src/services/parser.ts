...
export function parseMessagesUniversal(messages: Array<{ text: string }>) {
  ...
  for (const item of messages) {
    ...
    if (cur) flights.set(key, mergeFlights(cur, f));
    else flights.set(key, { ...f, dedup_key: key });
  }
...
function parseSingle(raw: string, logs: any[]) {
  ...
  // ---- Сегментация тегов ----
  const tagRegex =
    /\b(?:REG|TYP|STS|OPR|RMK|DOF|DEP|DEST|ADEPZ|ADARRZ|SID|ZZZZ|ATD|ATA)\b/gi;
  const tagPositions: Array<{ tag: string; index: number }> = [];
  ...
  const getBlock = (tag: string): string | null => {
  ...
  // ---- SID ----
  const sid =
    findFirst(text, /\bSID[\/\s:-]*([0-9]{6,})/i) ||
    findFirst(text, /\bSID\b[^\d]*?(\d{6,})/i);
  if (sid) f.sid = sid;

  // ---- DOF ----
  const dof =
    findFirst(text, /\bDOF[\/\s:-]*([0-9]{6})/i) ||
    findFirst(text, /-ADD\s+([0-9]{6})/i) ||
    findFirst(text, /-ADA\s+([0-9]{6})/i);
  if (dof) f.dof = dof6ToISO(dof);

  // ---- Время ----
  const depTime =
    findFirst(text, /\bZZZZ\s*([0-9]{4})\b/i) ||
    findFirst(text, /\b-ATD\s+([0-9]{4})\b/i);
  if (depTime) f.dep_time_utc = hhmmToTime(depTime);

  const arrTime =
    findNth(text, /\bZZZZ\s*([0-9]{4})\b/gi, 2) ||
    findFirst(text, /\b-ATA\s+([0-9]{4})\b/i);
  if (arrTime) f.arr_time_utc = hhmmToTime(arrTime);

  // Заголовки вида -UUFO1007 -UUDG0815 -ZZZZ1200
  const headerTime1 = findNth(text, /-(?:ZZZZ|[A-Z]{4})(\d{4})/g, 1);
  if (!f.dep_time_utc && headerTime1) f.dep_time_utc = hhmmToTime(headerTime1);

  const headerTime2 = findNth(text, /-(?:ZZZZ|[A-Z]{4})(\d{4})/g, 2);
  if (!f.arr_time_utc && headerTime2) f.arr_time_utc = hhmmToTime(headerTime2);

  // ---- TZ hint ----
  const tz =
    findFirst(text, /\b(UTC|Z)\b/i) ||
    findFirst(text, /\b([+-]\d{2}):?(\d{2})\b/i);
  if (tz) f.tz_hint = tz.toUpperCase().replace(/:/g, "");

  // ---- DEP/DEST ----
  const depStrict =
    getBlock("DEP") || getBlock("ADEPZ") || findFirst(text, /\bDEP[\/\s:-]*([0-9NSEW]+)/i);
  if (depStrict) {
  ...
  const destStrict =
    getBlock("DEST") || getBlock("ADARRZ") || findFirst(text, /\bDEST[\/\s:-]*([0-9NSEW]+)/i);
  if (destStrict) {
  ...
  if (!f.dep) {
    const depWithWords = findFirst(
      text,
      /\bDEP[\/\s:-]*[A-Z0-9 .()_\-]*?(?:\.\s*)?(\d{4,6}[NS]\d{5,7}[EW])/i
    );
    if (depWithWords) {
  ...
  if (!f.arr) {
    const destWithWords = findFirst(
      text,
      /\b(?:DEST|ADARRZ?)[\/\s:-]*[A-Z0-9 .()_\-]*?(?:\.\s*)?(\d{4,6}[NS]\d{5,7}[EW])/i
    );
    if (destWithWords) {
  ...
  if (!f.dep || !f.arr) {
    ...
    const candidates: string[] = [...coordsDMS];
    const decPairRe =
  ...
  const regsBlock =
    ...
    findFirst(
      text,
      /\bREG[\/\s:-]*([A-Z0-9 ,\-]+?)(?=\s+\b(?:DOF|DEP|DEST|ADEPZ|ADARRZ|TYP|STS|OPR|RMK|SID|ZZZZ|ATD|ATA)\b|[)\n]|$)/i
    );
  if (regsBlock) {
    f.reg = regsBlock
      .split(/[ ,]+/)
  ...
  if (typ) f.uav_type = typ.trim().toUpperCase();

  const sts = (getBlock("STS") || findFirst(text, /\bSTS[\/\s:-]*([A-Z]+)/i));
  if (sts) f.status = sts.trim().toUpperCase();
  ...
  if (zona || zonaCenter) {
    const zone: any = { source: "ZONA" };
    if (zona) zone.radius_km = Number(String(zona).replace(",", "."));
    if (zonaCenter) {
...
function normalizeRawOnce(s: string): string {
  return s
    .replace(/\r/g, "")
    .replace(/[ \t]+/g, " ")
    .replace(/[ \t]*\/[ \t]*/g, "/")
...
function inferType(text: string) {
  if (/^\(\s*SHR/i.test(text)) return "SHR";
  if (/^\(\s*DEP/i.test(text) || /-ATD\s+/i.test(text)) return "DEP";
  if (/^\(\s*ARR/i.test(text) || /-ATA\s+/i.test(text)) return "ARR";
  return "UNKNOWN";
}
...
function findNth(text: string, re: RegExp, n: number): string | null {
  let i = 0;
...
function parseCoordSafe(chunk: string): any | null {
  try {
...
function sanitizeOperatorName(s: string): string {
...
function validRegToken(tok: string): boolean {
  if (!/^[A-Z0-9-]{3,10}$/.test(tok)) return false;
  if (!/[A-Z]/.test(tok)) return false;
  if (/^\d{9,}$/.test(tok)) return false;
  return true;
}